...
function HomePage() {
  ...
  useEffect(() => {
    const fetchlistuser = async () => {
      try {
        ...
        // Merge the API data with the initialBooksData
        setBooks((prevBooks) => [...prevBooks, ...apiData]);
  ...
  const handleShelfChange = (id, newShelf) => {
    const updatedBooks = books.map(book => {
      if (book.id === id) {
        return { ...book, status: newShelf };  // Map shelf to status
      }
      return book;
    });
            ...
            <tbody>
              {filteredBooks.map((book, index) => (
                <tr key={index} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    {/* Update to actual average rate if available */}
                    {'★'.repeat(0)}{'☆'.repeat(5 - 0)}
                  </td>
                  <td className="px-4 py-2">
                    {/* Update to actual rating if available */}
                    {'★'.repeat(0)}{'☆'.repeat(5 - 0)}
                  </td>
                  <td className="px-4 py-2">
                    <select 
